...
const OutdoorIndoorButton = ({
  ...
  const handleOnClick = () => {
    if (divRef.current && radioRef.current) {
      ...
      radioRef.current.click(); // Simulate a click on the hidden radio input
    }
  };

  const handleChange = () => {
    if (radioRef.current) {
      setRoomType(radioRef.current.value); // Set the roomType when the radio input changes
    }
  };
  ...
  return (
    <div
      ...
      className={`flex h-16 w-3/4 cursor-pointer flex-row items-center justify-start gap-4 rounded-md border ${
        roomType === 'indoor' ? 'border-primary' : 'border-background-grey'
      } p-4 outline-offset-2 outline-primary transition-all ease-in hover:bg-background-grey focus:outline focus:outline-2`}
    >
      {children}
      <input
        type="radio"
