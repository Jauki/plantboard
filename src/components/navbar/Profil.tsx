...
const ProfilAnimated = ({ session }: { session: Session }) => {
  ...
  return (
    <motion.div
      className={`relative p-2 hover:bg-background-grey  rounded-md ${
    ...
    >
      ...
      >
        ...
        <div className='flex flex-col items-start'>
          ...
          <div className='text-xs text-foreground-grey font-light'>
            Plantdad 🌿
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div className='flex font-light flex-row gap-1 p-2 h-min text-sm'>
          ...
          <div className='w-10 flex place-content-end'>
            <Switch.Root className='group radix-state-checked:bg-primary radix-state-unchecked:bg-gray-200 dark:radix-state-unchecked:bg-gray-800 relative inline-flex  w-8 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus-visible:ring focus-visible:ring-purple-500 focus-visible:ring-opacity-75'>
              <Switch.Thumb className='group-radix-state-checked:translate-x-3 group-radix-state-unchecked:translate-x-0 pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-lg ring-0 transition duration-200 ease-in-out' />
            </Switch.Root>
          </div>
        </div>
        ...
        <Link
          href={'/logout'}
          className='flex font-light flex-row gap-1 p-2 h-min text-sm'
        >
          <div className='flex justify-center items-center'>
            <LogOut size={16} />
          </div>
